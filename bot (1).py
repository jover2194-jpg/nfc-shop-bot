...
async def broadcast_capture(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = []
    if product_key:
        buttons.append([InlineKeyboardButton("🛍️ Grab This Deal", callback_data=f"product_{product_key}")])
    buttons.append([InlineKeyboardButton("🔥 Join the VIP Group", url=COMMUNITY_URL)])
...
def main():
    ...
    def start_keepalive_server():
        """Tiny web server so Render sees an open port and free-tier uptime
        pingers (e.g. UptimeRobot) have something to hit to keep this awake."""
