...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: reply to any message (text, photo, whatever) with /broadcast
    to send that exact message to every subscriber (everyone who has sent
    /start). This is what powers a real 'News Feed' push, e.g. 'New product
    live!' with a photo, like other shop bots do.

    Optional: /broadcast <product_key> attaches a "Shop This Product" button
    that jumps straight into that product's page. Product keys: google,
    trustpilot, tripadvisor, whatsapp, telegram, social. E.g. /broadcast google
    """
    if not is_admin(update.effective_user.id):
        return
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "Reply to the message you want broadcast (text or photo) with "
            "/broadcast. Optionally add a product key to include a Shop "
            "button, e.g. /broadcast google — valid keys: "
            + ", ".join(PRODUCTS.keys())
        )
        return
    ...
    if context.args:
        ...
        if key not in PRODUCTS:
            ...
            return
        reply_markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🛍️ Shop This Product", callback_data=f"product_{key}")]]
        )
