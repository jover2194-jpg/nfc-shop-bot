...
def blockcypher_address_received(currency: str, address: str) -> int | None:
    """
    Return total_received in satoshis (or None on error).
    Free BlockCypher endpoint; optional token raises rate limits.
    """
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "browse":
        ...
        if received >= needed - tolerance:
            ...
            await query.answer(
                f"Still waiting — received {received / 1e8:.8f}, need {order['pay_amount']}. "
...
async def check_pending_payments_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    BlockCypher. When enough crypto has arrived, marks Paid and notifies both sides."""
