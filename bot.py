...
NOWPAYMENTS_PAY_CURRENCY = os.environ.get("NOWPAYMENTS_PAY_CURRENCY", "btc")  # Stripe, via BotFather
...
def init_db():
    ...
    # Migration: add destination_link to leads (the URL/number the card gets programmed to).
...
def nowpayments_create_payment(amount_gbp, order_id, description, pay_currency=None):
    """Create a crypto payment via NOWPayments. Returns the API response dict,
    or None if the API key isn't set or the request fails."""
    if not NOWPAYMENTS_API_KEY:
        return None
    pay_currency = pay_currency or NOWPAYMENTS_PAY_CURRENCY
    try:
        resp = requests.post(
            f"{NOWPAYMENTS_BASE_URL}/payment",
            headers={"x-api-key": NOWPAYMENTS_API_KEY, "Content-Type": "application/json"},
            json={
                "price_amount": amount_gbp,
                "price_currency": "gbp",
        ...
        if resp.status_code == 200:
            return resp.json()
        logger.error("NOWPayments create_payment failed: %s %s", resp.status_code, resp.text)
        return None
    except Exception:
        logger.exception("NOWPayments create_payment request error")
        return None


def nowpayments_get_status(payment_id):
    """Fetch current status for a NOWPayments payment_id. Returns dict or None."""
    if not NOWPAYMENTS_API_KEY:
        return None
    try:
        resp = requests.get(
        ...
        logger.exception("NOWPayments get_status request error")
        return None
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "browse":
        ...
        if not order or not order["payment_id"]:
            ...
            return
        status_data = nowpayments_get_status(order["payment_id"])
        if not status_data:
            await query.answer("Couldn't reach the payment network — try again shortly.", show_alert=True)
            return
        ...
        if status in ("finished", "confirmed"):
            ...
            await query.answer(f"Still waiting — status: {status}. I'll notify you automatically once it's paid.", show_alert=True)
        else:
...
async def ask_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    cur = conn.execute(
        """INSERT INTO leads (telegram_user_id, business_name, contact_name, phone, email, locations, product, bundle_size, destination_link, created_at)
           VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (
            update.effective_user.id,
            ud["business_name"],
    ...
    if NOWPAYMENTS_API_KEY:
        # Let the customer choose between BTC and LTC before creating the payment.
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("₿ Bitcoin (BTC)", callback_data="cryptocur_btc"),
                        InlineKeyboardButton("Ł Litecoin (LTC)", callback_data="cryptocur_ltc"),
                    ]
                ]
            ),
        )
...
async def select_crypto_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    payment = nowpayments_create_payment(
        amount_gbp, order_id, f"{ud['product']} x{ud['bundle_size']}", pay_currency
    )
    ...
    if payment and payment.get("pay_address"):
        ...
        qr_data = f"{actual_currency}:{pay_address}?amount={pay_amount}"
...
async def check_pending_payments_job(context: ContextTypes.DEFAULT_TYPE):
    """Runs on a timer. Checks every order still awaiting crypto payment and,
    if NOWPayments now shows it as paid, notifies the customer and admin."""
    if not NOWPAYMENTS_API_KEY:
        return
    conn = db()
    pending = conn.execute(
        "SELECT * FROM orders WHERE status='Awaiting crypto payment' AND payment_id IS NOT NULL"
    ).fetchall()
    conn.close()
    ...
    for order in pending:
        ...
        if status in ("finished", "confirmed"):
            conn = db()
            conn.execute("UPDATE orders SET status=? WHERE id=?", ("Paid", order["id"]))
            conn.commit()
            conn.close()
            try:
                await context.bot.send_message(
                ...
                logger.exception("Failed to notify customer for order %s", order["id"])
            if ADMIN_CHAT_ID:
                await context.bot.send_message(
                    ADMIN_CHAT_ID,
